...
$(function(){
    ...
    function cashlist()
    {
        ...
        $.ajax({
            url:'{:url('manage/cashlist')}'+'?page='+pid,
            type:'post',
            success:function(e){
                ...
                if(e.code===0)
                {
                    ...
                    $.each(e.data.data,function(k,d){
                        ...
                        if({$Mcash::CASH_APPLY}==d.status){
                            _html+='申请已提交,等待审核';
                            if(d.comment!=null) comment=d.comment;
                        }
                        if({$Mcash::CASH_FINANCE}==d.status){
                            _html+='申请已提交,等待审核';
                            if(d.comment!=null) comment=d.comment;
                        }
                        if({$Mcash::CASH_PASS}==d.status){
                            _html+='审核通过,等待发票';
                            if(d.comment!=null) comment=d.comment;
                        } 
                        if({$Mcash::CASH_REFUSE}==d.status){
                            _html+='驳回';
                            if(d.comment!=null) comment=d.comment;
                        } 
                        if({$Mcash::CASH_PAYED}==d.status){
                            _html+='已打款, 请查收';
                            if(d.comment!=null) comment=d.comment;
                        } 
