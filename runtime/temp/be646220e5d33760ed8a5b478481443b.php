<?php /*a:2:{s:53:"/data/www/csgx2/application/admin/view/bind_index.php";i:1566990717;s:56:"/data/www/csgx2/application/admin/view/public_header.php";i:1564456502;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8"/>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-inline">
                        <select id="s_3" lay-verify="required" class="layui-select">
                            ...
                            <option value="<?php echo htmlentities($Mbind::BIND_APPLY); ?>" >申请</option>
                            <option value="<?php echo htmlentities($Mbind::BIND_PASS); ?>" >通过</option>
                            <option value="<?php echo htmlentities($Mbind::BIND_REFUSE); ?>" >驳回</option>
                            <option value="<?php echo htmlentities($Mbind::BIND_UNBIND); ?>" >已解绑</option>
                        </select>
                    </div>
    ...
    layui.use(['layer', 'form', 'table', 'util', 'admin', 'formSelects'], function () {
        ...
        var insTb = table.render({
            ...
            cols: [[
                ...
                { templet: function (d) {
                       if(d.status==<?php echo htmlentities($Mbind::BIND_APPLY); ?> ) return '申请中';
                       if(d.status==<?php echo htmlentities($Mbind::BIND_PASS); ?> ) return '通过';
                       if(d.status==<?php echo htmlentities($Mbind::BIND_REFUSE); ?> ) return '驳回';
                       if(d.status==<?php echo htmlentities($Mbind::BIND_UNBIND); ?> ) return '已解绑';
                   }, title:'状态'},

                { templet: function(d){
                    if(d.status==<?php echo htmlentities($Mbind::BIND_APPLY); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="edit">审核</a>';
                    if(d.status==<?php echo htmlentities($Mbind::BIND_PASS); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="unbind">解绑</a>';
                    if(d.status==<?php echo htmlentities($Mbind::BIND_REFUSE); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="view">原因</a>';
                    if(d.status==<?php echo htmlentities($Mbind::BIND_UNBIND); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="unbinded">已解绑</a>';
                },title:'操作'}
            ]]
        });
            ...
            admin.open({
                ...
                success: function (layero, dIndex) {
                    ...
                    form.on('submit(unbindSubmit)', function (data) {
                        ...
                        layer.load(2);
                        $.post("<?php echo url('bind/unbind'); ?>", data.field, function (res) {
        ...
        function showEditModel(M) {
            if(M.status==<?php echo htmlentities($Mbind::BIND_PASS); ?>){
            ...
            admin.open({
                ...
                success: function (layero, dIndex) {
                    $(layero).children('.layui-layer-content').css('overflow', 'visible');
                    var url = "<?php echo url('bind/update'); ?>";
                    // 回显数据
                    if (M) {
                    ...
                    form.on('submit(modelObjSubmit)', function (data) {
                        ...
                        if( data.field.status!='' && data.field.status != M.status){
                            if(data.field.status==<?php echo htmlentities($Mbind::BIND_REFUSE); ?> ){
                                if(data.field.comment==''){
