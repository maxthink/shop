<?php /*a:1:{s:51:"/data/www/csgx2/application/wx/view/device_bind.php";i:1567061644;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<title>设备授权</title>
...
<body>
    <div class="dev-header">设备编码:<?php echo htmlentities($bindinfo['imei']); ?></div>
        ...
        <div class="layui-form container" style="display:none;">
            ...
            <div class="layui-form-mid layui-word-aux">请联系客服电话（400-106-2599）提供您的设备编号尾号<?php echo htmlentities(substr($bindinfo['imei'],-5)); ?>获取设备授权码。</div>
            <br/>
        ...
        <div class="bind_status" style="padding:8% 4%;text-align:center;">
            ...
            <div class="layui-form-item bind_refuse" style="display:none;">
                <p style="font-size:20px;" >
                    <span style="color:#FF5722;"><i class="layui-icon layui-icon-close" style="font-size:40px;"></i><br/>申请驳回,原因:<br/><?php echo htmlentities($bindinfo['comment']); ?></span>
                    <br/>
...
<script>
    ...
    layui.use(['form','jquery'], function(){
        ...
        var curr_uid=<?php echo htmlentities($bindinfo['current_uid']); ?>;
        var bind_uid=<?php echo htmlentities($bindinfo['uid']); ?>;
        var status=<?php echo htmlentities($bindinfo['status']); ?>;
        var status_last = <?php echo htmlentities($bindinfo['status_last']); ?>;

        if( <?php echo htmlentities($bind::BIND_UNBIND); ?> == status  )
        {
            $('.container').show();    //未绑定, 显示绑定
        }
        else if( <?php echo htmlentities($bind::BIND_APPLY); ?> == status  )
        {
        ...
        else if( <?php echo htmlentities($bind::BIND_REFUSE); ?> == status )
        {
            if(curr_uid==bind_uid){
        ...
        $('.showagain').on('click',function(){
            if(status_last!=status){
                if(status_last==<?php echo htmlentities($bind::BIND_APPLY); ?>){
                    $('.bind_refuse').hide(); 
                    $('.bind_apply_other').show();    //审核中, 不是自己提交
                }else{
                    $('.container').show();
