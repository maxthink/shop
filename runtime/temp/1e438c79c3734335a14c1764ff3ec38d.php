<?php /*a:2:{s:53:"/data/www/csgx2/application/admin/view/cash_index.php";i:1567151846;s:56:"/data/www/csgx2/application/admin/view/public_header.php";i:1564456502;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8"/>
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-inline">
                        <select id="s_2" lay-verify="required" class="layui-select">
                            ...
                            <option value="<?php echo htmlentities($Mcash::CASH_APPLY); ?>" >等待客服审核</option>
                            <option value="<?php echo htmlentities($Mcash::CASH_FINANCE); ?>" >等待财务审核</option>
                            <option value="<?php echo htmlentities($Mcash::CASH_PASS); ?>" >审核通过</option>
                            <option value="<?php echo htmlentities($Mcash::CASH_REFUSE); ?>" >驳回</option>
                            <option value="<?php echo htmlentities($Mcash::CASH_PAYED); ?>" >已打款</option>
                        </select>
                    </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="status" lay-filter="aihao">           
                    <option value="<?php echo htmlentities($Mcash::CASH_FINANCE); ?>" >通过</option>
                    <option value="<?php echo htmlentities($Mcash::CASH_REFUSE); ?>" >驳回</option>                    
                </select>
            </div>
        </div>
...
<script type="text/html" id="financeModel">
    <form id="financeForm" lay-filter="financeForm" class="layui-form model-form">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <select name="status" lay-filter="aihao">           
                    <option value="<?php echo htmlentities($Mcash::CASH_PASS); ?>" >通过</option>
                    <option value="<?php echo htmlentities($Mcash::CASH_REFUSE); ?>" >驳回</option>                    
                </select>
            </div>
        </div>
    ...
    layui.use(['layer', 'form', 'table', 'util', 'admin', 'formSelects'], function () {
        ...
        var insTb = table.render({
            ...
            cols: [[
                ...
                { templet: function (d) {
                       if(d.status==<?php echo htmlentities($Mcash::CASH_APPLY); ?> ) return '申请';
                       if(d.status==<?php echo htmlentities($Mcash::CASH_FINANCE); ?> ) return '财务审核';
                       if(d.status==<?php echo htmlentities($Mcash::CASH_PASS); ?> ) return '通过';
                       if(d.status==<?php echo htmlentities($Mcash::CASH_REFUSE); ?> ) return '驳回';
                       if(d.status==<?php echo htmlentities($Mcash::CASH_PAYED); ?> ) return '已打款';
                   }, title:'状态'},
                { templet: function(d){
                    if(d.status==<?php echo htmlentities($Mcash::CASH_APPLY); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="edit">待客服审核</a><a class="layui-btn layui-btn-xs" lay-event="show">数据核对</a>';
                    if(d.status==<?php echo htmlentities($Mcash::CASH_FINANCE); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="finance">待财务审核</a><a class="layui-btn layui-btn-xs" lay-event="show">数据核对</a>';
                    if(d.status==<?php echo htmlentities($Mcash::CASH_REFUSE); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="view">拒绝原因</a><a class="layui-btn layui-btn-xs" lay-event="show">数据核对</a>';
                    if(d.status==<?php echo htmlentities($Mcash::CASH_PASS); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="pay">等待发票</a><a class="layui-btn layui-btn-xs" lay-event="show">数据核对</a>';
                    if(d.status==<?php echo htmlentities($Mcash::CASH_PAYED); ?> ) return '<a class="layui-btn layui-btn-xs" lay-event="payinfo">已打款</a><a class="layui-btn layui-btn-xs" lay-event="show">数据核对</a>';
        ...
        $('#btnExport').click(function () {
            ...
            var form = $('<form method="POST" action="<?php echo url('cash/export'); ?>">');
        ...
        function showEditModel(M) {
            if( M.status==<?php echo htmlentities($Mcash::CASH_PASS); ?> ){
                layer.msg('已审核通过', {icon: 1});
                return;
            }
            var go = 0;
            $.ajax({
                url:'<?php echo url('auth/verify'); ?>',
                type:'post', 
            ...
            if(go===0){
                admin.open({
                    ...
                    success: function (layero, dIndex) {
                        ...
                        var url = "<?php echo url('cash/update'); ?>";
                        // 回显数据
                        if (M) {
                            M.money = M.money/100;
                        ...
                        form.on('submit(modelObjSubmit)', function (data) {
                            ...
                            if( data.field.status!='' && data.field.status != M.status){
                                if(data.field.status==<?php echo htmlentities($Mcash::CASH_REFUSE); ?> ){
        ...
        function showfinance(M) {
            ...
            if(go===0){
                admin.open({
                    ...
                    success: function (layero, dIndex) {
                        $(layero).children('.layui-layer-content').css('overflow', 'visible');
                        var url = "<?php echo url('cash/finance'); ?>";
                        // 回显数据
                        if (M) {
                            M.money = M.money/100;
